...
# https://github.com/talhasaruhan/spatial-pyramid-pooling
...
def spatial_pyramid_pooling(input_feature_maps, spatial_pyramid, dtype=np.float32):
    ...
    # N-C-W*H
    flattened_feature_maps = np.reshape(
        input_feature_maps, (batch_size, num_channels, -1)
    )
    ...
    for i in range(num_levels):
        ...
        ar = np.lib.stride_tricks.as_strided(
            ...
            (batch_size, num_channels, h, n_w, l),
            (
                sizeof_item * num_px * num_channels,
                sizeof_item * num_px,
                sizeof_item * w,
                sizeof_item * s,
                sizeof_item,
        ...
        ar = np.transpose(np.amax(np.asarray(ar), axis=4), axis=(0, 1, 3, 2)).copy()
        ...
        ar = np.lib.stride_tricks.as_strided(
            ar,
            (batch_size, num_channels, n_w, n_h, l),
            (
                sizeof_item * n_w * h * num_channels,
                sizeof_item * n_w * h,
                sizeof_item * h,
                sizeof_item * s,
                sizeof_item,
