# Created by Amir.H Ebrahimi at 2/22/21
...
class ApproximateKSVD(object):
    def __init__(self, n_components, max_iter=10, tol=1e-6,
                 transform_n_nonzero_coefs=None, dict_init=None, code_init=None):
        """
        Parameters
        ----------
        n_components:
    ...
    def fit(self, X):
        ...
        """
        if self.D is None:
            self.D = self._initialize(X)
        for i in range(self.max_iter):
            # gamma = self._transform(self.D, X)
            e = np.linalg.norm(X - self.Z.dot(self.D))
            if e < self.tol:
                break
            D, _ = self._update_dict(X, self.D, self.Z)
